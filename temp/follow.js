...
// 导入BLOB处理函数
import { processBlobs } from '@/utils/blobs';
...
export function getFollowList(params) {
  ...
  return request({
    ...
    console.log('客户跟进列表原始响应:', response);
    // 处理BLOB数据
    if (response && response.data && response.data.list) {
      response.data.list = processBlobs(response.data.list);
    }
    console.log('客户跟进列表处理后响应:', response);
    return response;
  }).catch(error => {
...
export function getFollowById(id) {
  ...
  return request({
    ...
    console.log('跟进详情原始响应:', response);
    // 处理BLOB数据
    if (response && response.data) {
      response.data = processBlobs(response.data);
    }
    console.log('跟进详情处理后响应:', response);
    return response;
  }).catch(error => {
